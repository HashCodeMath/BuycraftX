...
public class SecretSubcommand implements Subcommand {
    ...
    public void execute(final CommandSource sender, final String[] args) {
        ...
        if (!sender.equals(plugin.getServer().getConsoleCommandSource())) {
            sender.sendMessage(Component.text(plugin.getI18n().get("secret_console_only")).color(TextColor.fromHexString(ColorAPI.color("&c"))));
            return;
        }

        if (args.length != 1) {
            sender.sendMessage(Component.text(plugin.getI18n().get("secret_need_key")).color(TextColor.fromHexString(ColorAPI.color("&c"))));
            return;
        }
        ...
        plugin.getPlatform().executeAsync(() -> {
            ...
            try {
                ...
                sender.sendMessage(Component.text(plugin.getI18n().get("secret_does_not_work")).color(TextColor.fromHexString(ColorAPI.color("&c"))));
                return;
            }
            ...
            try {
                ...
                sender.sendMessage(Component.text(plugin.getI18n().get("secret_cant_be_saved")).color(TextColor.fromHexString(ColorAPI.color("&c"))));
            }

            sender.sendMessage(Component.text(plugin.getI18n().get("secret_success",
                    information.getServer().getName(), information.getAccount().getName())).color(TextColor.fromHexString(ColorAPI.color("&a"))));
